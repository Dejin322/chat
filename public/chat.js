...
const creatOne = (selector, el = document) => el.createElement(selector)
...
const userListDiv = findOne('.userListDiv');
...
S.on('user joined', (data) => {
  // получаем информацию о новом пользователе и добавляем его в список
  const { username, userId } = data;
  const userEl = document.createElement('li');
  userEl.classList.add('user-list-item');
